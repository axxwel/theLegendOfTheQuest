package{
	...
	public class DecorN3 extends MovieClip{
		...
		private var briqueL:int=480;
		private var briqueH:int=320;
		private var nbCol:int=4;
		private var nbLign:int=4;
		public var vitesseDG:int=2;
		public var vitesseHB:int=2;
		
		public dynamic function DecorN3 (pCarte:Array,pcarte_ligne_depart:int,pcarte_colonne_depart:int){
			carteNiveau=pCarte;
		...
		private function creerNiveau(){
			for(var L:Number=0;L<nbLign;L++){
				for(var C:Number=0;C<nbCol;C++){
					var brique:LibraryN3=new LibraryN3();
					brique.name="brique_"+L+"_"+C;
					brique.x=(briqueL*C)-briqueL;
					brique.y=(briqueH*L)-briqueH;
					brique.gotoAndStop(carteNiveau[L+carte_ligne_depart][C+carte_colonne_depart]);
		...
		public function deplace_fond_droite () {
				...
				for (var n:Number=0; n<nbLign; n++) {
  					...
  					conteneur.getChildByName(brique).x=conteneur.getChildByName(premiere_brique).x-briqueL;
   					MovieClip(conteneur.getChildByName(brique)).gotoAndStop(carteNiveau[(carte_ligne+n+carte_ligne_depart)][(carte_colonne-1+carte_colonne_depart)]);
   					ligne+=1;
				}
		...
		public function deplace_fond_gauche () {
				...
				for (var n:Number=0; n<nbLign; n++) {
   					...
   					MovieClip(conteneur.getChildByName(brique)).gotoAndStop(carteNiveau[(carte_ligne+n+carte_ligne_depart)][(carte_colonne+nbCol+carte_colonne_depart)]);
		...
		public function deplace_fond_bas() {
				...
				for (var n:Number=0; n<nbCol; n++) {
					...
					conteneur.getChildByName(brique).y=conteneur.getChildByName(premiere_brique).y-briqueH;
   					MovieClip(conteneur.getChildByName(brique)).gotoAndStop(carteNiveau[(carte_ligne-1+carte_ligne_depart)][(carte_colonne+n+carte_colonne_depart)]);
					colonne+=1;
				}
		...
		public function deplace_fond_haut () {
				...
				for (var n:Number=0; n<nbCol; n++) {
					...
					conteneur.getChildByName(brique).y=conteneur.getChildByName(derniere_brique).y+briqueH;	
   					MovieClip(conteneur.getChildByName(brique)).gotoAndStop(carteNiveau[(carte_ligne+nbLign+carte_ligne_depart)][(carte_colonne+n+carte_colonne_depart)]);
   					colonne+=1;
				}
